        ...
        def scannerHome = tool 'SonarQube Runner 2.8';
                 withSonarQubeEnv('DevOps Sonar') {
                   ...
                   withCredentials([[$class: 'StringBinding', credentialsId: 'sonarQube', variable: 'SONAR_LOGIN']]) {
                     sh "JOB_NAME=${jobname} && JOB_SHORT_NAME=${jobshortname} && set && ${scannerHome}/bin/sonar-scanner -Dsonar.host.url=${sonarHost} -Dsonar.login=${SONAR_LOGIN} -Dsonar.projectName=blackpearl"
           }
        }
    }
}
